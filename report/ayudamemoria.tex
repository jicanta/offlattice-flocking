...
\begin{tabular}{@{}llr@{}}
    ...
    Implementación (6--7)    & arquitectura y paso de simulación       & \SI{2}{min}\\
    Simulaciones (9--11)     & parámetros, observables, criterio de \teq & \SI{2}{min}\\
    Resultados (13--22)      & estándar, votante, clusters, tiempos    & \SI{6}{min}\\
    Conclusiones (24)        &                                          & \SI{1}{min}\\
\end{tabular}}
\par\vspace{0.4em}
{\small Las diapositivas \num{2}, \num{5}, \num{8}, \num{12} y \num{23} son
separadores de sección: se pasan sin hablar. Cada una de las \num{10}
diapositivas de resultados tiene unos \SI{35}{s}, así que hay tiempo para el
número clave y una frase, no más.}
\end{tcolorbox}
...
\diapo{6}{Arquitectura del motor}{\SI{50}{s}}
\begin{notas}
...
\diapo{7}{El paso de simulación}{\SI{70}{s}}
\begin{notas}
...
\diapo{11}{Del observable temporal al escalar}{\SI{55}{s}}
\begin{notas}
...
\diapo{13}{Modelo estándar: animaciones características}{\SI{40}{s}}
\begin{notas}
    \item \decir Dos corridas con la misma densidad y distinto ruido. Cada
          flecha es una partícula, con origen en su posición y color según el
          ángulo de su velocidad; la longitud es fija porque la rapidez es común
          a todas.
    \item \decir A la izquierda, $\eta = \SI{0.5}{rad}$: el color es casi
          uniforme, todas van para el mismo lado.
    \item \decir A la derecha, $\eta = \SI{4}{rad}$: los colores están
          repartidos, no hay dirección común.
...
\diapo{14}{Modelo estándar: evolución temporal}{\SI{45}{s}}
\begin{notas}
    ...
    \item \decir Con $\eta = \SI{0.5}{rad}$ sube a $\va \simeq 1$ en unos
          \SI{200}{s} y prácticamente no fluctúa.
    \item \decir Con $\eta = \SI{2}{rad}$ hay un plateau intermedio con
          fluctuación visible.
    \item \decir Con $\eta = \SI{5}{rad}$ nunca ordena: oscila en torno al piso
          de tamaño finito.
    ...
    \item \decir A la derecha, $S$ se queda pegado a $1$ en las tres: a
          ...
          inicial. Por eso hace falta bajar la densidad para estudiar clusters.
\end{notas}

\diapo{15}{Modelo de votante: animación y evolución temporal}{\SI{40}{s}}
\begin{notas}
    \item \decir Mismo $\eta = \SI{0.5}{rad}$ que ordenaba al estándar, y el
          votante no forma una bandada única.
    \item \decir Se ven \textbf{dominios} de dirección que se arman y se
          disuelven, y \va{} oscila con una amplitud comparable a su propia
          media.
    \item \decir Ésa es la firma de copiar a uno solo en vez de promediar: no
          hay ningún mecanismo que atenúe el ruido.
\end{notas}

\diapo{16}{Modelo estándar: polarización contra ruido}{\SI{45}{s}}
\begin{notas}
...
\diapo{17}{Modelo de votante: polarización contra ruido}{\SI{50}{s}}
\begin{notas}
    ...
    \item \preg \emph{¿Dónde está la comparación superpuesta?} En el informe, con
          los dos modelos en los mismos ejes a $\rho = \SI{2}{}$, \num{4} y
          \SI{8}{m^{-2}}. Acá se comparan estas dos diapositivas, que comparten
          ejes y colores.
...
\diapo{18}{Clusters: animaciones a baja densidad}{\SI{35}{s}}
\begin{notas}
    \item \decir Con las tres densidades de la consigna el grafo de vecinos está
          ...
          clusters a $\rho = 1/\pi$.
    \item \decir A esa densidad hay en promedio un vecino por partícula y $S$
...
\begin{notas}
    \item \decir Acá $S$ sí tiene transitorio propio: parte de $S \simeq 0.15$ y
          crece por agregación.
    \item \decir El mecanismo es simple: dos partículas que se encuentran se
          alinean, y al alinearse dejan de separarse. La unión sólo se deshace
          por el ruido.
    \item \nums \teq{} vale \SI{3.65e3}{s} en el caso $\eta = \SI{2}{rad}$ de
          la figura, unas diez veces el transitorio de \va{} a alta densidad; en
          otros puntos del barrido llega a \SI{5.6e3}{s}. Por eso estas corridas
          duran \SI{2e4}{s}.
    ...
    \item \preg Se muestra la curva promedio de las \num{20} realizaciones, no
          una sola, porque con $N = 32$ una realización aislada es demasiado
          ruidosa.
\end{notas}

\diapo{20}{Clusters: componente gigante contra ruido}{\SI{40}{s}}
\begin{notas}
    \item \decir Los dos paneles comparten el eje $[0,1]$ para que se comparen
          directamente.
    ...
    \item \decir O sea que \textbf{lo que fragmenta es el ruido, no la
          densidad}.
    \item \nums En $\eta = 0$: $S = \num{1.000(1)}$ y \num{0.991(40)}. El ruido
          de media componente vale \SI{2.74}{rad} y \SI{2.12}{rad}.
\end{notas}

\diapo{21}{Polarización contra componente gigante}{\SI{45}{s}}
\begin{notas}
    ...
    \item \nums Pearson \num{0.45} y \num{0.61} contra \num{0.985} y \num{0.994}.
          En el votante a $\rho = \SI{8}{m^{-2}}$ es incluso negativo
          (\num{-0.48}).
...
\diapo{22}{Tiempos de ejecución del CIM}{\SI{45}{s}}
\begin{notas}
